...
index_.to_csv('H_index.csv',header=False,index=False)

index=pd.read_csv('H_index.csv',header=None)
index=np.array(list(index.iloc[:,0]))

H_PseAAC = scipy.io.loadmat('T_H_PseAAC_8.mat')
H_PseAAC = H_PseAAC.get('H_PseAAC')
H_PseAAC = H_PseAAC[index, :]
H_PseAAC = pd.DataFrame(H_PseAAC)

H_AD = scipy.io.loadmat('T_H_AD_7.mat')
H_AD = H_AD.get('H_AD')
H_AD = H_AD[index, :]
H_AD = pd.DataFrame(H_AD)

H_AC = scipy.io.loadmat('T_H_AC_9.mat')
H_AC = H_AC.get('H_AC')
H_AC = H_AC[index, :]
H_AC = pd.DataFrame(H_AC)

H_CT = scipy.io.loadmat('T_H_CT.mat')
H_CT = H_CT.get('T_H_CT')
H_CT = H_CT[index, :]
H_CT = pd.DataFrame(H_CT)

H_LD = scipy.io.loadmat('T_H_LD.mat')
H_LD = H_LD.get('H_LD')
H_LD = H_LD[index, :]
H_LD = pd.DataFrame(H_LD)

H_MMI = scipy.io.loadmat('T_H_MMI.mat')
H_MMI = H_MMI.get('H_MMI')
H_MMI = H_MMI[index, :]
